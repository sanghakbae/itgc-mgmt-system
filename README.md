...
- `STORAGE_PROVIDER`
  - `local` 또는 `s3`. 로컬 개발 기본값은 `local`, 배포 환경에서는 `s3`를 사용합니다.
- `LOCAL_EVIDENCE_DIR`
...
증적 파일 업로드는 브라우저가 S3에 직접 접근하지 않고 서버 API(`/api/evidence/upload`)를 통해 처리합니다.

배포 환경에서 Access Key를 환경 변수로 넣지 않는 경우, 앱 서버가 실행되는 AWS 리소스의 IAM Role 또는 Pod Identity/IRSA가 S3 권한을 가져야 합니다.
